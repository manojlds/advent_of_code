mod days;

fn main() {
    let _ = days::day1::part1::solve();
    let _ = days::day1::part2::solve();
    
    let _ = days::day2::part1::solve();
    let _ = days::day2::part1_optimized::solve();
    let _ = days::day2::part2::solve();
    let _ = days::day2::part2_optimized::solve();

    let _ = days::day3::part1::solve();
    let _ = days::day3::part2::solve();

    let _ = days::day4::part1::solve();
    let _ = days::day4::part2::solve();

    let _ = days::day5::part1::solve();
    let _ = days::day5::part2::solve();

    let _ = days::day6::part1::solve();
    let _ = days::day6::part2::solve();

    let _ = days::day7::part1::solve();
    let _ = days::day7::part2::solve();

    let _ = days::day8::part1::solve();
    let _ = days::day8::part2::solve();
    
}
